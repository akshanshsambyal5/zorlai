...
export function UserMenu({ onOpenDashboard, onOpenAuth, className = '', compact = false }: UserMenuProps) {
  const { user, profile, loading, initialized, profileSyncing, isAuthenticated, signOut } =
    useAuthContext();
  ...
  if (showSkeleton) {
    return (
      <div
        className={`h-10 w-10 rounded-xl bg-sky-100/80 animate-pulse border border-sky-200/60 ${className}`}
  ...
  if (!isAuthenticated) {
    return (
      <button
        ...
        className={`btn-primary text-white text-xs font-medium px-4 py-2 min-h-[40px] rounded-xl inline-flex items-center gap-2 touch-manipulation transition-transform hover:scale-[1.02] active:scale-[0.98] ${className}`}
      >
  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      <motion.button
        ...
        whileTap={{ scale: 0.98 }}
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-2 pl-1 pr-2.5 py-1 rounded-xl glass-panel border border-sky-200/60 hover-glow transition-colors"
        aria-expanded={open}
        aria-haspopup="menu"
      ...
      >
        {display.avatar ? (
          <img
            src={display.avatar}
            alt=""
            className="w-8 h-8 rounded-lg object-cover ring-2 ring-sky-300/80"
            referrerPolicy="no-referrer"
          />
        ) : (
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-sky-500 to-blue-700 flex items-center justify-center text-[10px] font-bold text-white shadow-sm">
            {initials}
          </div>
        )}
        {!compact && (
          <span className="text-xs font-semibold text-slate-800 max-w-[120px] truncate hidden md:block">
            {display.name}
          </span>
        )}
        {profileSyncing ? (
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 mt-2 w-60 rounded-2xl glass-panel-strong py-2 shadow-xl z-[60] border border-slate-200/80"
            role="menu"
          >
