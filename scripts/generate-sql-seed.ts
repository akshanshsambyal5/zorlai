...
for (const t of TOOL_SEEDS) {
  ...
  '${esc(t.id)}', '${esc(t.slug)}', '${esc(t.name)}', '${esc(t.tagline)}', '${esc(t.description)}', '${esc(t.icon)}', '${esc(t.url)}', '${esc(t.logo_url)}', '${esc(t.category_id)}', ${arr(t.tags)}, ${t.votes}, 0, '${t.pricing}', ${t.rating}, ${t.reviews_count}, ${t.is_trending}, ${t.is_featured}, ${arr(t.features)}, 'published'
